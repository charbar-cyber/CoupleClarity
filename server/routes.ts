...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/therapy-sessions', isAuthenticated, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Set date range to fetch recent entries (last 2 weeks)
      const twoWeeksAgo = new Date();
      twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
      
      // Get user's private journal entries from the last 2 weeks
      const userPrivateEntries = await storage.getUserJournalEntries(req.user.id, true);
      const recentUserPrivateEntries = userPrivateEntries.filter(entry => 
        new Date(entry.createdAt) >= twoWeeksAgo
      ).map(entry => ({
        title: entry.title,
        content: entry.content,
      ...
      // Get shared journal entries from both partners
      const sharedEntries = await storage.getSharedJournalEntries(req.user.id, partnerId);
      
      // Split into user's shared entries and partner's shared entries
      const userSharedEntries = sharedEntries
        .filter(entry => entry.userId === req.user!.id && new Date(entry.createdAt) >= twoWeeksAgo)
        .map(entry => ({
          title: entry.title,
          content: entry.content,
          emotions: entry.emotions || undefined,
      ...
      const partnerSharedEntries = sharedEntries
        .filter(entry => entry.userId === partnerId && new Date(entry.createdAt) >= twoWeeksAgo)
        .map(entry => ({
          title: entry.title,
          content: entry.content,
          emotions: entry.emotions || undefined,
      ...
      const conflictThreads = await storage.getConflictThreads(partnership.id);
      const recentConflictThreads = conflictThreads
        .filter(thread => new Date(thread.createdAt) >= twoWeeksAgo)
        .map(thread => {
      ...
      // Generate therapy session using OpenAI
      const therapySession = await openai.generateTherapySession(
        allUserEntries,
  ...
  app.get('/api/therapy-sessions', isAuthenticated, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Get all therapy sessions for this partnership
  ...
  app.post('/api/therapy-sessions', isAuthenticated, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const recentConflictThreads = conflictThreads
        .filter(thread => new Date(thread.createdAt) >= twoWeeksAgo && thread.messages && thread.messages.length > 0)
        .map(thread => {
          // Get messages for each conflict thread
          const messages = thread.messages.map(msg => ({
            author: msg.userId === req.user!.id ? req.user!.firstName : 'Partner',
  ...
  // Get list of therapy sessions for the current user's partnership
  app.get('/api/therapy-sessions', isAuthenticated, async (req: Request & { user?: User }, res: Response) => {
    try {
      // Get the partnership for the current user
      const partnership = await storage.getPartnershipByUser(req.user!.id);
  ...
  // Update a therapy session (add notes, mark as reviewed)
